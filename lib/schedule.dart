...
class Schedule {
  ...
  String prt() {
    String s = '';
    Items.forEach(
        (f) => s += f.HabitCoin.Name + ', ' + f.DaysOfWeek.toString());
    return s;
  }
...
//print(d.toString());
    ...
    s.FirstDate = DateTime.fromMillisecondsSinceEpoch(
        int.parse(j['firstDate'].toString()));
    s.LastDate = DateTime.fromMillisecondsSinceEpoch(
        int.parse(j['lastDate'].toString()));
